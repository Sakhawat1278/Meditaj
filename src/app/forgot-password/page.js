    ...
    <main className="min-h-screen bg-white flex selection:bg-black selection:text-white">
        ...
        <div className="relative z-10 w-full p-16 flex flex-col justify-between h-full text-white">
          <div>
            <div className="flex items-center gap-3">
              <div className="grid grid-cols-2 gap-0.5"><div className="w-2.5 h-2.5 rounded-full bg-white"/><div className="w-2.5 h-2.5 rounded-full bg-white"/><div className="w-2.5 h-2.5 rounded-full bg-white"/><div className="w-2.5 h-2.5 rounded-full bg-white"/></div>
      ...
      <section className="w-full lg:w-1/2 min-h-screen flex flex-col overflow-y-auto bg-white">
        <div className="w-full px-8 py-6 flex items-center justify-between sticky top-0 z-50 bg-white/90 backdrop-blur-md">
          <Link href="/login" className="flex items-center gap-2 group text-slate-400 hover:text-black transition-colors">
            <div className="w-9 h-9 border border-slate-200 rounded-full flex items-center justify-center group-hover:border-black transition-all"><ChevronLeft size={16}/></div>
            <span className="text-[10px] font-bold uppercase tracking-widest">Back</span>
          </Link>
          <button onClick={() => switchLanguage(locale === 'en' ? 'bn' : 'en')} className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 hover:text-black transition-colors"><Globe size={12}/>{locale.toUpperCase()}</button>
        </div>
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                <motion.div key="s1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                  <div>
                    <h2 className="text-2xl font-extrabold tracking-tighter text-black uppercase mb-3">Forgot Password</h2>
                  ...
                  <form className="space-y-5" onSubmit={handleSendCode}>
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        ...
                        <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@email.com" className="w-full h-11 pl-11 pr-4 bg-white border border-slate-200 rounded-full focus:outline-none focus:border-black transition-all text-[13px] font-medium text-black placeholder:text-slate-200" />
                      </div>
                    </div>
                    <button type="submit" disabled={isSubmitting} className="group relative flex items-center justify-end h-11 bg-black text-white rounded-full hover:bg-slate-800 transition-all overflow-hidden disabled:opacity-50">
                      {isSubmitting ? <div className="w-32 h-full flex items-center justify-center"><Loader2 className="animate-spin" size={18}/></div> : (
                        <motion.div initial={{ width: 44 }} animate={{ width: "auto" }} transition={{ delay: 0.1 }} className="flex items-center">
                          ...
                          <div className="w-9 h-9 border border-white/30 rounded-full flex items-center justify-center mr-1 shrink-0 group-hover:bg-white group-hover:text-black transition-all"><ArrowRight size={16}/></div>
                        </motion.div>
                      )}
                    </button>
                  </form>
              ...
              {step === 2 && (
                <motion.div key="s2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-10 flex flex-col items-center">
                  <div className="text-center">
                    ...
                    <h2 className="text-2xl font-extrabold tracking-tighter text-black uppercase mb-2">Verify Code</h2>
                  ...
                  <div className="flex gap-4">
                    {otp.map((digit, i) => (
                      <input key={i} id={`otp-${i}`} type="text" maxLength={1} value={digit} onChange={(e) => handleOtpChange(i, e.target.value)} className="w-12 h-12 text-center text-xl font-black bg-white border border-slate-200 rounded-full focus:border-black focus:ring-4 focus:ring-black/5 outline-none transition-all shadow-sm" />
                    ))}
                  </div>
                  <div className="flex flex-col items-center gap-6">
                    <button onClick={handleVerifyPin} disabled={isSubmitting || otp.some(d => !d)} className="group relative flex items-center justify-end h-11 bg-black text-white rounded-full hover:bg-slate-800 transition-all overflow-hidden disabled:opacity-50">
                    ...
                    <button onClick={handleResend} disabled={isResendDisabled} className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-black transition-colors">{isResendDisabled ? "Wait 30s" : "Resend Code"}</button>
                  </div>
                </motion.div>
              )}
              ...
              {step === 3 && (
                <motion.div key="s3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                  <div>
                    <h2 className="text-2xl font-extrabold tracking-tighter text-black uppercase mb-3">New Password</h2>
                    <div className="w-8 h-0.5 bg-black rounded-full" />
                  </div>
                  <form onSubmit={handleResetPassword} className="space-y-5">
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <input type={showPassword ? "text" : "password"} required value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full h-11 px-5 bg-white border border-slate-200 rounded-full focus:outline-none focus:border-black transition-all text-[13px] font-medium text-black"/>
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-black">{showPassword ? <EyeOff size={16}/> : <Eye size={16}/>}</button>
                      </div>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">Confirm</label>
                      <input type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full h-11 px-5 bg-white border border-slate-200 rounded-full focus:outline-none focus:border-black transition-all text-[13px] font-medium text-black"/>
                    </div>
                    <button type="submit" disabled={isSubmitting} className="group relative flex items-center justify-end h-11 bg-black text-white rounded-full hover:bg-slate-800 transition-all overflow-hidden disabled:opacity-50">
                      {isSubmitting ? <div className="w-48 h-full flex items-center justify-center"><Loader2 className="animate-spin" size={18}/></div> : (
                        <motion.div initial={{ width: 44 }} animate={{ width: "auto" }} transition={{ delay: 0.1 }} className="flex items-center">
