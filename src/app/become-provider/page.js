                ...
                <form onSubmit={handleBecomeProvider} className="space-y-8 focus:outline-none">
                  <section className="space-y-6">
                    <div className="flex items-center gap-3">
                      <div className="w-7 h-7 rounded-full bg-[#1e4a3a] flex items-center justify-center text-white text-[10px] font-black tracking-widest flex-shrink-0">01</div>
                      ...
                      {(() => {
                        ...
                        return (
                          ...
                          <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                            <div className="relative group w-16 h-16 rounded-full border border-dashed border-slate-400 bg-white flex flex-col items-center justify-center hover:border-emerald-500 transition-all cursor-pointer overflow-hidden shrink-0">
                              {data.profileImage ? (
                                <>
                  ...
                  {selectedProvider === 'doctor' && (
                    ...
                    <section className="space-y-6 pt-10 border-t border-slate-100">
                      <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-full bg-[#1e4a3a] flex items-center justify-center text-white text-[10px] font-black tracking-widest flex-shrink-0">02</div><h2 className="text-[11px] font-black text-[#1e4a3a] uppercase tracking-[0.3em] border-b-2 border-[#1e4a3a] pb-1">Clinical Credentials & Registry</h2></div>
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-8 gap-y-5">
                        ...
                        <div className="space-y-2"><label className="input-label-premium">License Expiry</label><DatePicker icon={Calendar} value={doctorData.bmdcExpiry} onChange={(val) => setDoctorData({...doctorData, bmdcExpiry: val})} placeholder="YYYY-MM-DD" minDate={new Date()} /></div>
                        ...
                        <div className="lg:col-span-3 space-y-2"><label className="input-label-premium">Academic Accolades (Press Enter to add)</label><div className="min-h-[3rem] p-3 bg-slate-50 border border-slate-200 rounded-[30px] transition-all focus-within:bg-white focus-within:border-emerald-500 shadow-inner shadow-[#1e4a3a]/5"><div className="flex flex-wrap gap-2 mb-2">{degrees.map((deg, i) => (<span key={i} className="px-3 py-1.5 bg-[#1e4a3a] text-white text-[10px] font-black rounded-full flex items-center gap-2 uppercase tracking-widest border border-[#1e4a3a] shadow-lg">{deg} <X size={12} className="cursor-pointer text-emerald-500" onClick={() => setDegrees(degrees.filter(d => d !== deg))} /></span>))}<input type="text" value={degreeInput} onChange={(e) => setDegreeInput(e.target.value)} onKeyDown={handleDegreeAdd} placeholder="MBBS, FCPS, MD..." className="flex-grow bg-transparent outline-none text-[13px] font-bold px-2 uppercase tracking-widest" /></div></div></div>
                        <div className="lg:col-span-3 space-y-2"><label className="input-label-premium">Professional Clinical Bio</label><textarea rows={3} value={doctorData.bio} onChange={e => setDoctorData({...doctorData, bio: e.target.value})} placeholder="Describe your specialization, treatment philosophy, and professional background for patient review..." className="w-full p-5 bg-white border border-slate-300 rounded-[30px] focus:outline-none focus:border-emerald-600 text-[13px] font-bold uppercase tracking-tight placeholder:text-slate-300 shadow-sm" /></div>
                      </div>
                    </section>
                    <section className="space-y-6 pt-10 border-t border-slate-100">
                      <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-full bg-[#1e4a3a] flex items-center justify-center text-white text-[10px] font-black tracking-widest flex-shrink-0">03</div><h2 className="text-[11px] font-black text-[#1e4a3a] uppercase tracking-[0.3em] border-b-2 border-[#1e4a3a] pb-1">Specialization & Billing</h2></div>
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-8 gap-y-5">
                        <div className="space-y-3"><label className="input-label-premium">Clinical Specialization</label><div className="flex flex-wrap gap-2 p-1">{Object.keys(specialties).map(spec => (<button key={spec} type="button" onClick={() => toggleSpecialty(spec)} className={`px-4 py-2 rounded-full text-[9px] font-black border transition-all uppercase tracking-widest ${doctorData.specialty.includes(spec) ? 'bg-emerald-600 border-emerald-600 text-white shadow-lg' : 'bg-slate-50 border-slate-200 text-slate-400 hover:border-[#1e4a3a] hover:text-[#1e4a3a]'}`}>{spec}</button>))}</div></div>
                         <div className="lg:col-span-2 space-y-3"><label className="input-label-premium">Treatment Codes Coverage</label><div className="min-h-[140px] p-6 bg-slate-50 border border-slate-200 rounded-[30px] space-y-5 shadow-inner">{doctorData.specialty.length === 0 ? <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.15em] italic text-center py-6">Pick specialties to enable clinical services...</p> : doctorData.specialty.map(spec => (<div key={spec} className="space-y-2"><div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /><span className="text-[9px] font-black text-[#1e4a3a] uppercase tracking-[0.2em]">{spec} Diagnostic Scope</span></div><div className="flex flex-wrap gap-2">{(specialties[spec] || []).map(s => <button key={s} type="button" onClick={() => toggleService(s)} className={`px-3 py-1.5 rounded-full text-[8px] font-black transition-all border uppercase tracking-widest ${doctorData.services.includes(s) ? 'bg-[#1e4a3a] border-[#1e4a3a] text-white' : 'bg-white border-slate-200 text-slate-500 hover:border-[#1e4a3a] hover:text-[#1e4a3a]'}`}>{s}</button>)}</div></div>))}</div></div>
                  ...
                  <section className="pt-10 flex flex-col items-center border-t border-slate-200 pb-10 gap-6">
                    ...
                    <button type="submit" disabled={isLoading} className="w-[300px] h-11 bg-[#1e4a3a] hover:bg-black text-white rounded-full font-bold text-[12px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 transition-all active:scale-95 disabled:opacity-50 shadow-xl shadow-[#1e4a3a]/10">
