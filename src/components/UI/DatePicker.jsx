...
export default function DatePicker({ value, onChange, placeholder = "Select a date", label, className = "", error, icon: Icon = CalendarIcon, position = "bottom", minDate }) {
    ...
    <div className={`relative w-full space-y-1.5 ${className}`} ref={containerRef}>
      {label && (
        <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 ml-1 flex items-center gap-2">
          {Icon && <Icon size={10} className="text-purple-500" />}
          <span>{label}</span>
          {error && <span className="text-rose-500 lowercase font-normal italic ml-auto">{error}</span>}
        </label>
      )}
      <div 
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute z-50 left-0 p-4 bg-white rounded-2xl border border-[#1e4a3a]/10 w-[280px] shadow-2xl ${position === 'top' ? 'bottom-full mb-2' : 'top-full mt-2'}`}
          >
            <div className="flex items-center justify-between mb-4">
              <button 
                ...
                className={`w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-600 transition-colors ${showYearPicker ? 'opacity-0' : ''}`}
              >
                <ChevronLeft size={16} />
              </button>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button 
                  ...
                  className={`px-3 py-1 rounded-full text-[13px] font-extrabold tracking-tight transition-all ${showYearPicker ? 'bg-[#1e4a3a] text-white' : 'text-[#1e4a3a] hover:bg-slate-100'}`}
                >
                  {currentYear}
                </button>
              </div>
                ...
                >
                  {years.map(year => (
                    <button
                      ...
                      className={`h-9 flex items-center justify-center rounded-full text-[12px] font-bold transition-all
                      ${currentYear === year 
                    ...
                    {days.map(day => {
                      ...
                      return (
                        <button
                          ...
                          disabled={disabled}
                          className={`w-8 h-8 flex items-center justify-center rounded-full text-[12px] font-bold transition-all mx-auto
                          ${isSelected 
                            ? 'bg-[#1e4a3a] text-white' 
                            : disabled 
